...
If you already have KO annotations for your genomes (e.g., generated by another tool), you can skip the DIAMOND search step by providing a folder of per-genome annotation files with `--ko_input`:

```
checkm2 predict --threads 30 --input <folder_with_bins> --output-directory <output_folder> --ko_input <folder_with_ko_annotations>
```

Each annotation file should be named `{genome_name}.{any_extension}` (matching the genome filename without extension) and contain one KEGG Ortholog (KO) ID per line with no header:
