...
If you already have KO annotations for your genomes (e.g., generated by another tool), you can skip the DIAMOND search step by providing a folder of per-genome annotation files with `--ko_input`. In this case, `--input` should point to a folder of **protein (amino acid) FASTA files** instead of genome sequences:

```
checkm2 predict --threads 30 --input <folder_with_protein_files> --output-directory <output_folder> --ko_input <folder_with_ko_annotations>
```

Each annotation file should be named `{genome_name}.{any_extension}` (matching the protein filename without extension) and be a CSV file with a header row containing `gene_id` and `ko` columns:
...
The `gene_id` values must correspond to sequence names in the protein FASTA file. All gene IDs listed in the annotation file must be present in the protein file; any genome where this check fails will be skipped. Genes with no KO assignment can be included with an empty `ko` field. Genes absent from the annotation file (i.e., unannotated genes) are also permitted — a warning will be logged but quality prediction will proceed.
